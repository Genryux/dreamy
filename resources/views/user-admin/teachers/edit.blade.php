...
@section('breadcrumbs')
    <nav aria-label="Breadcrumb" class="flex flex-row justify-between items-center mb-2 mt-2">
        <ol class="flex items-center gap-1 text-sm text-gray-700">
            ...
            <li>
                <span class="block text-gray-900">Edit Teacher</span>
            </li>
        </ol>
    </nav>
@endsection
...
@section('header')
    <div class="flex flex-row justify-between items-start text-start px-[14px] py-2">
        <div>
            <h1 class="text-[24px] font-black text-gray-900">Edit Teacher: {{ $teacher->getFullNameAttribute() }}</h1>
            <p class="text-[14px] text-gray-600 mt-1">Update teacher information and account details</p>
        </div>
            ...
            <div class="border-b border-gray-200 pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="flex flex-col">
                        ...
                        <input type="tel" name="contact_number" id="contact_number" value="{{ old('contact_number', $teacher->contact_number) }}"
                ...
                <div class="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <div class="flex items-center">
                        <i class="fi fi-sr-info text-blue-500 mr-2"></i>
                        <span class="text-sm text-blue-700 font-medium">Employee ID: {{ $teacher->employee_id }}</span>
                    </div>
                    <p class="text-xs text-blue-600 mt-1">Employee ID cannot be changed after creation.</p>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="flex flex-col">
                        ...
                        <select name="specialization" id="specialization"
                            ...
                            <option value="Mathematics" {{ old('specialization', $teacher->specialization) == 'Mathematics' ? 'selected' : '' }}>Mathematics</option>
                            <option value="Science" {{ old('specialization', $teacher->specialization) == 'Science' ? 'selected' : '' }}>Science</option>
                            <option value="English" {{ old('specialization', $teacher->specialization) == 'English' ? 'selected' : '' }}>English</option>
                            <option value="Filipino" {{ old('specialization', $teacher->specialization) == 'Filipino' ? 'selected' : '' }}>Filipino</option>
                            <option value="Social Studies" {{ old('specialization', $teacher->specialization) == 'Social Studies' ? 'selected' : '' }}>Social Studies</option>
                            <option value="Physical Education" {{ old('specialization', $teacher->specialization) == 'Physical Education' ? 'selected' : '' }}>Physical Education</option>
                            <option value="Arts" {{ old('specialization', $teacher->specialization) == 'Arts' ? 'selected' : '' }}>Arts</option>
                            <option value="Technology" {{ old('specialization', $teacher->specialization) == 'Technology' ? 'selected' : '' }}>Technology</option>
                        </select>
