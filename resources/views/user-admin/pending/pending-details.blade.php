...
@section('modal')
    <div id="modal-bg"
        class="absolute bottom-0 left-0 bg-[#0f111c]/40 h-0 w-full z-20 ease-in-out duration-150 overflow-hidden">
        ...
        <div class="flex items-center justify-center h-screen w-screen">
            ...
            <div id="modal" class="bg-[#f8f8f8] flex flex-col rounded-md w-[40%]" onclick="event.stopPropagation()">
                ...
                <form action="/set-interview/{{ $form->applicants_id }}" method="post" id="interview-form"
                ...
                <div class="flex justify-end px-4 py-3 space-x-1">
                    ...
                    <button form="interview-form" name="action" value="accept-with-schedule"
    ...
    <x-header-container>
        <div class="px-[16px] py-[16px] flex flex-row items-center justify-between space-x-2">
            <div class="flex flex-row items-center space-x-2">
                ...
                <h2 class="text-[16px]"> <span class="text-[#0f111c]/80">Applicant Details:</span><span
                        class="opacity-100 font-medium  font-bold"> {{ $form->full_name }} </span></h2>
            </div>
            <div class="flex flex-row items-center space-x-1">
                <button type="submit" name="action" form="interview-form" value="accept-only"
                    class="border border-[#1e1e1e]/15 bg-[#199BCF] px-2 py-1 rounded-md text-[#f8f8f8] text-[14px] font-bold">Accept
                    Only</button>
                <button id="accept-btn"
                    class="border border-[#1e1e1e]/15 bg-[#199BCF] px-2 py-1 rounded-md text-[#f8f8f8] text-[14px] font-bold">Accept
                    & Schedule</button>
                <button id="reject-btn"
                    class="border border-[#1e1e1e]/15 px-2 py-1 rounded-md text-[#0f111c]/80 text-[14px] font-bold">Reject</button>
            </div>
        </div>
    </x-header-container>
@endsection
                ...
                <tbody>
                    ...
                    <tr class="opacity-[0.87]">
                        <td class="px-4 py-2 text-[14px] border-b border-r border-[#1e1e1e]/15 w-1/2">Grade Level to
                            Enroll:<span class="font-bold"> {{ $form->grade_level }}</span></td>
                    ...
                    <tr class="opacity-[0.87]">
                        <td class="px-4 py-2 text-[14px] border-b border-r border-[#1e1e1e]/15 w-1/2">Birthdate:<span
                                class="font-bold"> {{ $form->birthdate }}</span></td>
