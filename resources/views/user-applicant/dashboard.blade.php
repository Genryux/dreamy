    ...
    <div class="flex flex-col h-full py-8 px-6 md:px-2 gap-6">
        ...
        @if ($activeEnrollmentPeriod)
            ...
            <div class="text-center">
                ...
                @if ($activeEnrollmentPeriod->status == 'Closed')
                ...
                @elseif ($activeEnrollmentPeriod->status == 'Paused')
                    ...
                    <p id="db-text" class="text-[18px] m-6 md:text-[20px] font-bold">Enrollment period is currently closed. At this time, we are not accepting any new applications.</p>
                        
                @else

                    <p id="db-text" class="text-[18px] md:text-[24px] font-bold">Welcome to Dreamy School' Online Registration for 2025
                        <p id="zspan" class="text-[16px] md:text-[18px] font-medium">Please click the button below to fill out the form.</p> 
                    </p>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.Echo.channel('updating-enrollment-period-status').listen('EnrollmentPeriodStatusUpdated', (event) => {
            ...
            if (event.enrollmentPeriod.status == 'Paused') {
                ...
                document.getElementById('db-text').innerText = 'Enrollment period is now officially closed, and we will no longer be accepting any applications.';
                document.getElementById('btn-container').classList.add('hidden');
